...
- [ ] **Nouveau** : les routes `app/api/agent_routes.py` ne vérifient pas le rôle/la session côté serveur (voir section ci-dessus) — ajouter une dépendance d'auth réelle (session signée) avant toute exposition au-delà du réseau interne.
- [ ] Monter l'authentification (`OAuthService`) sur toutes les routes sensibles ; fail-closed si allowlist vide (le login local fonctionne, mais rien ne bloque encore un rôle usurpé côté API).
